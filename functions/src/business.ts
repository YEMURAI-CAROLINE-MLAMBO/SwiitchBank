...
async function hasBusinessRole(userId: string, businessId: string, requiredRole: string): Promise<boolean> {
  try {
...
async function isBusinessMember(userId: string, businessId: string): Promise<boolean> {
    try {
      const user = await admin.auth().getUser(userId);
      const customClaims = user.customClaims || {};
      return Object.keys(customClaims).some(claimKey =>
          claimKey.startsWith(`business_${businessId}`)
      );
    } catch (error) {
...
export const createBusinessAccount = functions.https.onCall(async (data, context) => {
...
export const removeTeamMember = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const user = await admin.auth().getUser(userId);
    const currentCustomClaims = user.customClaims || {};
    const newCustomClaims = { ...currentCustomClaims };
...
export const updateTeamMemberRole = functions.https.onCall(async (data, context) => {
...
export const createCorporateWallet = functions.https.onCall(async (data, context) => {
