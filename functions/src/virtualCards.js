...
exports.createVirtualCard = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    return {
      success: true,
      cardId: marqetaCard.token,
      lastFour: marqetaCard.last_four,
      expirationDate: marqetaCard.expiration_time,
    };
  } catch (error) {
    ...
    throw new functions.https.HttpsError(
      'internal',
      'Failed to create virtual card. Please try again.'
    );
  }
});
    ...
    await transactionRef.set(
      {
        ...
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        settledAt: transactionData.settlement_date
          ? new Date(transactionData.settlement_date)
