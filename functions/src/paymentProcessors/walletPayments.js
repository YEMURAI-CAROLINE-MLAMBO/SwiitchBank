...
class WalletPaymentService {
    ...
    async processWalletPayment(paymentData, amount, currency, platform) {
        try {
            const paymentIntent = await stripe.paymentIntents.create({
                ...
                payment_method_data: platform === 'google_pay' ? {
                    type: 'card',
                    card: {
                        token: paymentData.paymentMethodData.tokenizationData.token
                    }
                } : undefined,
                payment_method: platform === 'apple_pay' ? paymentData.token : undefined,
    ...
    processWalletPayment = functions.https.onCall(async (data, context) => {
        if (!context.auth) {
        ...
        try {
            const result = await this.processWalletPayment(paymentData, amount, currency, platform);
